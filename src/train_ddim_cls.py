...
if __name__ == '__main__':
    ...
    eval_set = TSEDataset(meta_dir=args.data_path+'eval.csv', data_dir=args.data_path, subset='val', length=10,
                          use_timbre_feature=args.use_timbre_feature, timbre_path=args.timbre_path,
                          mel_length=1000)
    ...
    # unet.load_state_dict(torch.load('149.pt')['model'])
    ...
    if accelerator.is_main_process:
        eval_ddim(autoencoder, unet, logmel_val, noise_scheduler, eval_loader, args, accelerator.device,
                  epoch='test', ddim_steps=args.num_infer_steps, eta=1)
